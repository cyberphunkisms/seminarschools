(() => {
  ...
  function renderCommonFilters() {
    ...
    root.innerHTML = order.map(key => axisMarkup(key)).join('');
  }

  function renderResearchFamilies() {
    const commonRoot = $('#pmdResearchCommonFilters');
    if (commonRoot) {
      commonRoot.innerHTML = COMMON_AXES.filter(key => axes.has(key)).map(key => axisMarkup(key)).join('');
    }
...
<section class="pmd-intent"><h3>${TEXT.observanceTitle}</h3><p>${TEXT.observanceText}</p><div class="pmd-intent-actions"><button type="button" data-intent-axis="what" data-intent-value="event:seasonal-observance">${TEXT.seasonal} (${seasonalCount.toLocaleString(UI_LOCALE)})</button><button type="button" data-intent-axis="what" data-intent-value="event:ritual-observance">${TEXT.ritual} (${ritualCount.toLocaleString(UI_LOCALE)})</button></div></section>
  ...
  function eventActions(event) {
    ...
    // Every public record, including an undated monitoring record, owns a
    // stable internal detail route. The payload repeats this as a `details`
    // action so either layer can fail independently without removing the
    // visitor's path; `seen` makes the rendered action singular.
  ...
  function renderResults() {
    ...
    if (mobile) {
      mobile.textContent = COPY.compactResults(visibleCount);
      mobile.setAttribute('aria-label', COPY.viewResults(visibleCount));
    }
    const sort = $('#pmdSort');
