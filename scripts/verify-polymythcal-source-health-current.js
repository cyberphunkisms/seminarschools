...
if (!/RequestsFetcher\(\s*timeout\s*=\s*request_timeout\s*\)/s.test(protest)) {
  failures.push('protest harvester is missing its bounded RequestsFetcher timeout');
}
...
if (failures.length) {
  console.error('CURRENT POLYMYTHCAL SOURCE-HEALTH INTEGRITY FAILED');
  failures.forEach(failure => console.error(` - ${failure}`));
  process.exit(1);
}

console.log(
  'CURRENT POLYMYTHCAL SOURCE-HEALTH INTEGRITY PASSED — 25% overall quorum, critical-source floor, '
    + '304 refusal, stream binding, complete deterministic paid-work skips, and weekly cadence.',
);
