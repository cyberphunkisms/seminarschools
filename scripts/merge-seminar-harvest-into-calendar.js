 ...
 * the public calendar. Exact occurrences and explicit stable aliases refresh;
 * same-title same-day sessions remain distinct.
 */
const crypto = require('crypto');
const fs = require('fs');
const path = require('path');
const ROOT = path.resolve(__dirname, '..');
const SEMINARS = process.env.POLYMYTHCAL_SEMINARS_PATH
  ? path.resolve(process.env.POLYMYTHCAL_SEMINARS_PATH)
  : path.join(ROOT, 'seminars', 'events.json');
...
function derivedAliases(event) {
  ...
  if (!bases.length) bases.push(`fallback::${source}::${title}::${words(event.organizer).slice(0, 120)}`);
...
function intersects(left, right) {
  const a = matchAliases(left);
  for (const value of matchAliases(right)) if (a.has(value)) return true;
  return false;
}
...
function sameOccurrence(left, right) {
  ...
  if (!intersects(left, right) || String(left.date || '').slice(0, 10) !== String(right.date || '').slice(0, 10)) {
    return sameExactEvent(left, right);
  }
  const generic = value => /^(?:|locationtbd|locationtba|tbd|tba|online|virtual|toronto|montreal|montréal|kingston)$/i.test(words(value));
...
const preparedHarvest = harvested
  .filter(e => e?.title && e?.date && e?.source_url)
  .map(prepare);
const batchAliasCounts = new Map();
for (const incoming of preparedHarvest) {
  ...
  if (existingIndex < 0) {
    ...
    if (old.review_status === 'manual' && incoming.review_status !== 'manual') {
      ...
      events[existingIndex] = {
        ...
        lifecycle_status: rescheduled ? 'rescheduled' : (incoming.lifecycle_status || old.lifecycle_status || 'active'),
