...
const ui=readText('polymythseminars/index.html'), revamp=readText('js/polymythcal-revamp.js'), detail=readText('scripts/build-polymythcal-audit13.py');
for(const value of requiredFormats) assert(ui.includes(`value="${value}"`),`UI lacks Participation facet ${value}`);
for(const marker of ['participationFormats','classifyParticipationFormats','event._participationFormats','participatory_formats','participation_mode']) assert(revamp.includes(marker),`Participation runtime missing ${marker}`);
