...
const TODAY = resolveSiteBuildDate({root: ROOT, override: null});
...
function sitemapEligible(event, day = TODAY) {
  const start = eventStart(event);
  return eventIndexable(event, day)
    && /^\d{4}-\d{2}-\d{2}$/.test(start)
    && start <= dateOneYearAfter(day);
}
...
function main() {
  ...
  const expectedSitemapEvents = new Set(
    events.filter(event => sitemapEligible(event)).map(event => SITE + eventRoute(event))
  );
