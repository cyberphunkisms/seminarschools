...
/**
 * Apply the current manifest-owned Polymythcal asset token to active source
 * HTML immediately before localized routes bind their English source hashes.
 * Upstream generators use the manifest-owned token; historical audit evidence
 * and programs are intentionally outside this narrow surface.
 */
const fs = require('fs');
...
const PREVIOUS_ASSET = '20260726-audit47';
const EXPECTED_RELEASE =
  '2026-07-26-site-audit48-external-validation-interoperability-final';
...
if (manifest.release_id !== EXPECTED_RELEASE) {
  throw new Error(`Audit 48 release stamp cannot target ${manifest.release_id}`);
}
if (currentAsset !== '20260726-audit48') {
  throw new Error(`Audit 48 release stamp cannot use ${currentAsset}`);
}
...
for (const target of [...new Set(files)].sort()) {
  ...
  const temporary = `${target}.audit48-tmp`;
...
if (stale.length) {
  console.error(`AUDIT48 RELEASE STAMP CHECK FAILED — ${stale.length} stale pages`);
  stale.slice(0, 100).forEach(relative => console.error(` - ${relative}`));
  process.exit(1);
}
console.log(
  `AUDIT48 RELEASE STAMP ${CHECK ? 'CHECK ' : ''}PASSED — `
