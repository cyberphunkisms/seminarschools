...
const EXPECTED_RELEASE_ID = '2026-07-24-site-audit37-source-health-runtime-polish-final';
const RELEASE = JSON.parse(fs.readFileSync(path.join(ROOT, 'RELEASE_MANIFEST.json'), 'utf8'));
const REPORTS = [
  {
    path: 'data/polymythcal-audit37/interaction-design-browser-audit.json',
    release: 'PolymythCAL Audit 37 end-to-end interaction and design verification',
  },
  {
    path: 'data/polymythcal-audit37/entry-pages-browser-audit.json',
    release: 'PolymythCAL Audit 37 dedicated entry-page browser verification',
  },
  {
    path: 'data/polymythcal-audit37/wcag22-browser-audit.json',
    release: 'PolymythCAL Audit 37 end-to-end',
    standard: 'WCAG 2.2 AA',
  },
  {
    path: 'data/audit37-route-browser/route-resilience-browser-audit.json',
    release: 'Audit37 route resilience and recovery',
  },
  {
    path: 'scripts/reports/audit37-project-failure-stress.json',
    release: 'Audit37 project failure-mode and stress',
  },
];
...
for (const expected of REPORTS) {
  ...
  if (report.audit !== 37) failures.push(`${rel} is not stamped Audit 37`);
...
if (failures.length) {
  console.error('AUDIT 37 FRESH BROWSER EVIDENCE FAILED');
  failures.forEach(failure => console.error(` - ${failure}`));
  process.exit(1);
}

console.log(
  `AUDIT 37 FRESH BROWSER EVIDENCE PASSED — ${REPORTS.length} current-release browser reports are fresh and complete.`,
);
