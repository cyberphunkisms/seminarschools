#!/usr/bin/env python3
"""Verify the exact September 5 truthful-completion to feminism-gate transition.
...
BASELINE = SITE_ROOT / "data" / "futureproofing" / "sep5-package-contents-baseline.json"
CONTRACT = (
    SITE_ROOT
    / "data"
    / "futureproofing"
    / "sep5-truthful-sep5-feminism-preservation-contract.json"
)

BASELINE_SHA256 = "5595b80acf723af19b5ec019f55390d39571940eeb1bb9feffdb3c48ee803b64"
PREDECESSOR_RELEASE_ID = (
    "core-coreplus-mephistodata-truthful-completion-enforcement-complete-2026-09-05"
)
PREDECESSOR_GENERATED_AT = "2026-09-05T18:00:00Z"
PREDECESSOR_FILE_COUNT = 21201
PREDECESSOR_TOTAL_BYTES = 560340609
GOVERNED_PREDECESSOR_COUNT = 21177
OUTER_PREDECESSOR_COUNT = 23
TRANSITION_ID = "sep5-truthful-to-sep5-feminism-academic-research-gate"
CONTRACT_SCHEMA = "seminar-schools-sep5-feminism-preservation-v1"
CONTENT_POLICY_VERSION = "sep5-feminism-fixed-point-policy-v1"
SITE_PREFIX = "SITE_PACKAGE/"
EXCLUDED_PREDECESSOR_PATH = "PACKAGE_CONTENTS_SHA256.json"
CONTRACT_RULE = (
    "Every governed September 5 truthful-completion SITE_PACKAGE and outer "
...
POLICY_ROWS = (
    ...
    },
    {
        ...
        "writer": "reviewed September 5 feminism research-gate transition seal",
        "semantic_gate": "python3 scripts/verify-sep5-feminism-base-preservation.py",
    },
)
...
def verify_historical_bytes() -> None:
    ...
    assert sha256(BASELINE) == BASELINE_SHA256, (
        "September 5 truthful-completion baseline is not the exact predecessor manifest"
    )


def baseline_document() -> dict[str, Any]:
    assert sha256(BASELINE) == BASELINE_SHA256, (
        "September 5 truthful-completion package manifest hash drifted"
    )
...
def observed_transition() -> dict[str, Any]:
    ...
    current_files, selection_stats = collect_package_files(
        SITE_ROOT,
        DELIVERY_ROOT / ".sep5-feminism-preservation-probe.zip",
    ...
    delivery_files, _delivery_selection = collect_package_files(
        DELIVERY_ROOT,
        DELIVERY_ROOT / ".sep5-feminism-delivery-preservation-probe.zip",
        excluded_top_level={"qa"},
    )
    ...
    assert outer_delivery["deleted"] == [], (
        "September 5 feminism transition contains an unauthorized outer deletion"
    )
    ...
    assert [row["path"] for row in deleted] == list(AUTHORIZED_PUBLIC_DELETIONS), (
        "September 5 feminism transition contains an unauthorized predecessor deletion"
    )
    ...
    for relative in EVENT_DATA_PATHS:
        ...
        event_data.append(
            {
                **current_exact,
                "unchanged_from_sep5_truthful_completion": (
    ...
    return {
        ...
        "baseline_manifest": {
            "path": "data/futureproofing/sep5-package-contents-baseline.json",
...
def validate_expected(observed: dict[str, Any], contract: dict[str, Any]) -> None:
    assert set(contract) == {
        ...
        "expected_transition",
    }, "September 5 feminism transition contract has unrecognized or missing fields"
    assert contract.get("schema") == CONTRACT_SCHEMA
    assert contract.get("contract_version") == "2026-09-05.2"
    assert contract.get("status") == "sealed", (
        "September 5 feminism preservation contract is unsealed"
    )
    ...
    assert isinstance(expected, dict), "September 5 feminism transition is absent"
    assert observed == expected, (
        "September 5 truthful-completion to feminism-gate transition drifted"
    )
    assert [row["path"] for row in observed["deleted"]] == list(AUTHORIZED_PUBLIC_DELETIONS)
    assert all(
        row["unchanged_from_sep5_truthful_completion"]
    ...
    assert [
        ...
        if row["path"] == (
            "UPDATE_SOURCES/"
            "ML_STAR_UPDATE_SOURCE_FEMINISM_ACADEMIC_RESEARCH_GORGONIFICATION_2026-09-05.md"
        )
    ] == [
        "UPDATE_SOURCES/ML_STAR_UPDATE_SOURCE_FEMINISM_ACADEMIC_RESEARCH_GORGONIFICATION_2026-09-05.md"
    ], "the exact outer feminism research-gate provenance source addition is absent"
...
def main() -> None:
    ...
    if args.seal:
        if args.contract.resolve() != CONTRACT.resolve():
            raise SystemExit(
                "--seal writes only the canonical September 5 feminism transition contract"
            )
        write_contract(observed)
        print(
            "SEP5-TRUTHFUL→SEP5-FEMINISM PRESERVATION SEALED — "
    ...
    print(
        "SEP5-TRUTHFUL→SEP5-FEMINISM PRESERVATION PASSED — "
