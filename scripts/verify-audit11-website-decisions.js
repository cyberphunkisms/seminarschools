...
if(data.rules?.ultimate_application_cv){
  const records=(data.ultimate_application_cv?.experience_sections||[]).flatMap(section=>section.records||[]);
  const bumi=records.find(r=>r.organization==='BUMI Festival (Toronto area)');
  if(!bumi)fail.push('BUMI missing from ultimate application CV');
  if(!bumi?.focus?.includes('volunteer-events'))fail.push('BUMI missing from volunteer focus');
}else{
...
for(const t of ['ss-col-title','<h2 class="ss-col-title">',"about: 'About'","[copy.about, '/about/']"])has('js/footer.js',t);
...
for(const t of ['apply-final8-website-polish.py','apply-final9-mephistodata-website-hardening.py','apply-audit10-remaining-website-fixes.py'])has('scripts/build-saul-cv-professional.py',t);
lacks('scripts/build-saul-cv-professional.py','apply-audit11-website-decisions.py');
has('scripts/verify-audit10-remaining-website.js','/js/footer.js?v=20260805-predeploy-audit');
const active=[];
function walk(d){for(const e of fs.readdirSync(d,{withFileTypes:true})){if(['public','docs','.git'].includes(e.name)||isGeneratedDependencyDirectory(e.name))continue;const p=path.join(d,e.name);if(e.isDirectory())walk(p);else if(/\.(html|xml|txt)$/.test(e.name))active.push(p)}}
walk(root);
...
if(exists('public/about/index.html')){
  has('public/index.html','data-homepage-contract="business-card-project-web-v1"');
