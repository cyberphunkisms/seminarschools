...
check(
  (searchBuilder.match(/catalog\.css\?v=20260724-audit40/g) || []).length >= 2,
  'Teacher Resources and Methodology generated pages do not share the Audit 40 catalog token',
);
...
const runtimeBrowser = read('scripts/audit40-browser-runtime.py');
includesAll('Audit 40 DM-board browser coverage', runtimeBrowser, [
...
if (failures.length) {
  console.error('AUDIT 40 RUNTIME EFFICIENCY FAILED');
  failures.forEach(failure => console.error(` - ${failure}`));
  process.exit(1);
}

console.log(
  `AUDIT 40 RUNTIME EFFICIENCY PASSED — ${sourceFiles.length} source HTML/CSS/JS files `
