...
with open(summary_path, 'a', encoding='utf-8') as out:
    ...
    out.write(f"Publication: `{status.get('publication_status','unknown')}`  \n")
    out.write(f"Agent exit code: `{status.get('agent_exit_code',status.get('exit_code','?'))}`  \n")
    out.write(f"Agent failure kind: `{status.get('agent_failure_kind',status.get('failure_kind','unknown'))}`  \n")
