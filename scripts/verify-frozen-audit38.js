...
const MANIFEST_PATH = path.join(ROOT, 'data', 'audit38-frozen-sha256.json');
...
try {
  ...
  console.error(`AUDIT 38 FROZEN EVIDENCE FAILED — ${error.message}`);
  process.exit(1);
}
...
if (failures.length) {
  console.error('AUDIT 38 FROZEN EVIDENCE FAILED');
  failures.forEach(failure => console.error(` - ${failure}`));
  process.exit(1);
}

console.log(
  'AUDIT 38 FROZEN EVIDENCE PASSED — 14 report/gate files remain byte-identical; '
    + '838/32/422, 41/213, 644/25/7, 1,139, 24, 124, and 3,550 remain '
    + 'historical baselines rather than live ceilings.',
);
