...
const NOW = new Date();
const ROUTES = {
  writingclub: { band:'club', label:'Writing Club', countText:n=>`${n} writing competitions` },
  writingkids: { band:'kids', label:'Writing Kids', countText:n=>`${n} writing competitions` },
  writingjuniors: { band:'juniors', label:'Writing Juniors', countText:n=>`${n} writing competitions` },
  writingteens: { band:'teens', label:'Writing Teens', countText:n=>`${n} writing competitions` },
  writinggrads: { band:'grads', label:'Writing Grads', countText:n=>`${n} writing competitions` }
};
...
function setWritingButtons(html, activeBand){
  for (const [slug, info] of Object.entries(ROUTES)) {
    ...
    const label = info.label.replace('Writing ', '');
    const re = new RegExp(`<a href="/${slug}/" data-writing="${info.band}"(?: class="on")?(?: aria-current="page")?>${label}</a>`,'g');
