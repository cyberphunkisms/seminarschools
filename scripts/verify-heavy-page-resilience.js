...
const expected=['polymyth/methodologylist/index.html','teacherresources/index.html','polymythseminars/index.html','aa/index.html','polymyth/campaigncodex/index.html'];
...
for(const route of shortcuts){
  ...
  if((data.events||[]).length>180) fail(`${route} fallback is too large for a route-specific shortcut: ${(data.events||[]).length}`);
...
console.log('HEAVY PAGE RESILIENCE PASSED — large pages are render-contained and shortcut pages carry route-specific fallbacks.');
