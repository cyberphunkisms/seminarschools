...
for (const good of ['Choose what this CV shows','Pick one or more areas. The PDF uses what you pick.','data-cat="all" class="on" id="allBtn">Full CV</button>','if (active.has(cat)) active.delete(cat);','else active.add(cat);']) {
...
if (!/return '0\.155'/.test(apply)) errors.push('scripts/apply-visible-geometry.js: Polymythcal intensity was not raised');
if (errors.length) {
  console.error('HUMAN-FACING COPY / CV SELECTION CHECK FAILED');
  errors.forEach(e => console.error(' - ' + e));
  process.exit(1);
}
console.log('HUMAN-FACING COPY CHECK PASSED — Polymythcal instructions are practical, Saul CV areas are multi-select, and geometry remains animated.');
