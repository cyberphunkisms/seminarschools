#!/usr/bin/env python3
"""Verify the exact August 31 to September 3 SITE_PACKAGE transition.
...
authorized. Every successor-only path is also
...
BASELINE = SITE_ROOT / "data" / "futureproofing" / "aug31-package-contents-baseline.json"
CONTRACT = SITE_ROOT / "data" / "futureproofing" / "aug31-sep3-preservation-contract.json"

BASELINE_SHA256 = "40e0f94e5bf88486d6d16c003ed6e01132e22a366675243b5b8fbe84b260773a"
PREDECESSOR_RELEASE_ID = "core-coreplus-mephistodata-activation-enforcement-complete-2026-08-31"
PREDECESSOR_GENERATED_AT = "2026-08-31T15:00:00Z"
PREDECESSOR_FILE_COUNT = 21187
PREDECESSOR_TOTAL_BYTES = 549857479
GOVERNED_PREDECESSOR_COUNT = 21165
TRANSITION_ID = "aug31-to-sep3-bottom-up-definition-enforcement"
CONTRACT_SCHEMA = "seminar-schools-aug31-sep3-preservation-v1"
CONTENT_POLICY_VERSION = "aug31-sep3-fixed-point-policy-v1"
SITE_PREFIX = "SITE_PACKAGE/"
EXCLUDED_PREDECESSOR_PATH = "PACKAGE_CONTENTS_SHA256.json"
CONTRACT_RULE = (
    "Every governed August 31 SITE_PACKAGE path remains byte-identical unless its "
...
POLICY_ROWS = (
    ...
    {
        "path": "data/futureproofing/aug31-sep3-preservation-contract.json",
        "classification": "successor_contract_self_reference",
        "writer": "reviewed August 31 to September 3 transition seal",
        "semantic_gate": "python3 scripts/verify-sep3-base-preservation.py",
    },
)
...
def verify_historical_bytes() -> None:
    ...
    assert sha256(BASELINE) == BASELINE_SHA256, (
        "August 31 baseline is not the exact predecessor package manifest"
    )


def baseline_document() -> dict[str, Any]:
    assert sha256(BASELINE) == BASELINE_SHA256, "August 31 package manifest hash drifted"
...
def observed_transition() -> dict[str, Any]:
    ...
    current_files, selection_stats = collect_package_files(
        SITE_ROOT,
        DELIVERY_ROOT / ".sep3-preservation-probe.zip",
    ...
    assert [row["path"] for row in deleted] == list(AUTHORIZED_PUBLIC_DELETIONS), (
        "September 3 transition contains an unauthorized predecessor deletion"
    )
    ...
    return {
        ...
        "baseline_manifest": {
            "path": "data/futureproofing/aug31-package-contents-baseline.json",
...
def validate_expected(observed: dict[str, Any], contract: dict[str, Any]) -> None:
    assert set(contract) == {
        ...
        "expected_transition",
    }, "August 31 to September 3 contract has unrecognized or missing fields"
    assert contract.get("schema") == CONTRACT_SCHEMA
    assert contract.get("contract_version") == "2026-09-03.1"
    assert contract.get("status") == "sealed", "September 3 preservation contract is unsealed"
    ...
    assert isinstance(expected, dict), "September 3 preservation transition is absent"
    assert observed == expected, "August 31 to September 3 exact preservation transition drifted"
    assert [row["path"] for row in observed["deleted"]] == list(AUTHORIZED_PUBLIC_DELETIONS)
    assert all(row["unchanged_from_aug31"] for row in observed["event_data"]), (
        "Polymythcal event corpus or safe public projections changed from August 31"
    )
...
def main() -> None:
    ...
    if args.seal:
        if args.contract.resolve() != CONTRACT.resolve():
            raise SystemExit("--seal writes only the canonical September 3 preservation contract")
        write_contract(observed)
        print(
            "AUG31→SEP3 PRESERVATION SEALED — "
    ...
    print(
        "AUG31→SEP3 PRESERVATION PASSED — "
