...
assert(netlify.includes('publish = "public"') && (/build-public-deploy\.js/.test(netlify) || (netlify.includes(`command = "${requiredNetlifyBuild}"`) && /build-public-deploy\.js/.test(canonicalBuild))), 'Netlify publish directory must be generated /public through the canonical build so the full zip root remains archival and safe.');
