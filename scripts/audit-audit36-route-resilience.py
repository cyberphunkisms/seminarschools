#!/usr/bin/env python3
"""Fresh Chromium evidence for Audit 36 route polish and failure recovery."""
...
SITE_ROOT = ROOT / os.environ.get("AUDIT36_SITE_ROOT", "public")
OUT = ROOT / "data" / "audit36-route-browser"
...
def audit_representative_routes(browser, base_url: str) -> None:
    for viewport_name, viewport in VIEWPORTS:
        ...
        context.add_init_script(
            """(() => {
              window.__audit36CLS = 0;
              try {
                new PerformanceObserver(list => {
                  for (const entry of list.getEntries()) {
                    if (!entry.hadRecentInput) window.__audit36CLS += entry.value;
                  }
        ...
        for route_path, label in ROUTES:
            print(f"AUDIT36 {viewport_name}: {label}", flush=True)
            ...
            cls = float(page.evaluate("Number(window.__audit36CLS || 0)"))
            metric = {
                "route": route_path,
                "label": label,
...
def main() -> None:
    ...
    phase = os.environ.get("AUDIT36_PHASE", "all").strip().lower()
    if phase not in {"all", "routes", "stress"}:
        raise SystemExit("AUDIT36_PHASE must be all, routes, or stress")
