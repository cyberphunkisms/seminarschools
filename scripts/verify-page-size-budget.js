...
function walk(dir,acc=[]){ for(const e of fs.readdirSync(dir,{withFileTypes:true})){ if(['.git','node_modules','.netlify'].includes(e.name)) continue; const full=path.join(dir,e.name); if(e.isDirectory()) walk(full,acc); else if(e.isFile() && e.name.endsWith('.html')) acc.push(full); } return acc; }
...
for(const f of walk(ROOT)){
  const rel=path.relative(ROOT,f).replace(/\\/g,'/'); const size=fs.statSync(f).size;
