...
CAPTURE_SCREENSHOTS = os.environ.get('AUDIT42_INHERITED_SCREENSHOTS', '').strip() == '1'
OUT = ROOT / "data" / "audit42-inherited-browser" / "interaction"
...
with sync_playwright() as p:
    ...
    print("AUDIT42 desktop start", flush=True)
    ...
    context.close()
    print("AUDIT42 desktop complete", flush=True)

    # French interface and dynamic labels.
    print("AUDIT42 French start", flush=True)
    ...
    context.close()
    print("AUDIT42 French complete", flush=True)
    ...
    print("AUDIT42 mobile start", flush=True)
    ...
    print("AUDIT42 mobile complete", flush=True)

    # Forced-colour resilience.
    print("AUDIT42 forced-colour start", flush=True)
...
report = {
    ...
    "checks_failed": sum(1 for item in RESULTS if not item["passed"]),
    "results": RESULTS,
}
