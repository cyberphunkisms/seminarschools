...
OUT_JSON = ROOT / "data" / "audit42-inherited-browser" / "wcag" / "wcag22-browser-audit.json"
OUT_MD = ROOT / "data" / "audit42-inherited-browser" / "wcag" / "wcag22-browser-audit.md"
...
def preserve_report_mtime(path: Path) -> None:
    value = os.environ.get("AUDIT42_INHERITED_REPORT_OUTPUT_MTIME")
    if not value:
        return
    try:
        moment = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as error:
        raise SystemExit("AUDIT42_INHERITED_REPORT_OUTPUT_MTIME must be a valid timestamp") from error
