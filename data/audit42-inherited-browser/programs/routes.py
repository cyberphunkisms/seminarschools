#!/usr/bin/env python3
"""Fresh Chromium evidence for Audit 42 route polish and failure recovery."""
...
SITE_ROOT = ROOT / os.environ.get("AUDIT42_INHERITED_SITE_ROOT", "public")
...
def audit_representative_routes(browser, base_url: str) -> None:
    for viewport_name, viewport in VIEWPORTS:
        ...
        context.add_init_script(
            """(() => {
              window.__audit42CLS = 0;
              try {
                new PerformanceObserver(list => {
                  for (const entry of list.getEntries()) {
                    if (!entry.hadRecentInput) window.__audit42CLS += entry.value;
                  }
        ...
        for route_path, label in ROUTES:
            print(f"AUDIT42 {viewport_name}: {label}", flush=True)
            ...
            cls = float(page.evaluate("Number(window.__audit42CLS || 0)"))
            metric = {
                "route": route_path,
                "label": label,
...
def main() -> None:
    ...
    phase = os.environ.get("AUDIT42_INHERITED_PHASE", "all").strip().lower()
    if phase not in {"all", "routes", "stress"}:
        raise SystemExit("AUDIT42_INHERITED_PHASE must be all, routes, or stress")
