#!/usr/bin/env python3
"""Fresh Chromium audit for Audit 42 runtime continuity and repaired deep links."""
...
def runtime_metrics(page: Page) -> dict[str, Any]:
    return page.evaluate(
        """() => {
          ...
          const audit = window.__audit42Runtime || {};
...
def route_matrix(browser, base_url: str) -> None:
    for viewport_name, viewport in VIEWPORTS:
        ...
        context.add_init_script(
            """window.__audit42Runtime = {cls: 0, shifts: [], longTasks: []};
            try {
              new PerformanceObserver(list => {
                for (const entry of list.getEntries()) {
                  if (!entry.hadRecentInput) {
                    window.__audit42Runtime.cls += entry.value;
                    window.__audit42Runtime.shifts.push({
                      value: entry.value,
            ...
            try {
              new PerformanceObserver(list => {
                for (const entry of list.getEntries()) {
                  window.__audit42Runtime.longTasks.push(entry.duration);
                }
        ...
        "held_direction_decisions": [
            {
                ...
                "reason": "Audit 42 preserves the archive's layout-shift repair. Replacing its full-payload rendering to eliminate the remaining startup long task would change the delivery architecture.",
            }
        ],
        "environment_limits": [
